...
class TestFeatureEngineering:
    ...
    def test_failure_rate_zero_when_no_count(self, service, mock_db):
        ...
        mock_record = Mock()
        mock_record.source_ip = "10.0.0.1"
        mock_record.volume = 5
        mock_record.total_count = 0
        mock_record.failed_count = 0
        mock_record.unique_domains = 1
        mock_record.first_seen = datetime(2024, 1, 1)
        mock_record.last_seen = datetime(2024, 1, 2, 12, 0)
    ...
    def test_ip_data_metadata(self, service, mock_db):
        ...
        mock_record = Mock()
        mock_record.source_ip = "172.16.0.1"
        mock_record.volume = 25
        mock_record.total_count = 50
        mock_record.failed_count = 10
        mock_record.unique_domains = 2
        mock_record.first_seen = datetime(2024, 1, 1)
        mock_record.last_seen = datetime(2024, 1, 15)
...
class TestModelTraining:
    ...
    def _generate_mock_records(self, count):
        """Generate mock database records for training"""
        records = []
        for i in range(count):
            record = Mock()
            record.source_ip = f"192.168.{i // 256}.{i % 256}"
            record.volume = np.random.randint(1, 100)
            record.total_count = np.random.randint(50, 500)
            record.failed_count = np.random.randint(0, record.total_count)
            record.unique_domains = np.random.randint(1, 10)
            record.first_seen = datetime(2024, 1, 1) + timedelta(days=i % 30)
            record.last_seen = datetime(2024, 1, 15) + timedelta(days=i % 30, hours=i % 24)
            records.append(record)
        return records

    def test_train_with_sufficient_data(self, service, mock_db):
        """Test model training succeeds with enough data"""
        records = self._generate_mock_records(150)
        mock_db.query.return_value.join.return_value.filter.return_value.group_by.return_value.all.return_value = records
    ...
    def test_train_stores_model_in_db(self, service, mock_db):
        ...
        records = self._generate_mock_records(120)
        mock_db.query.return_value.join.return_value.filter.return_value.group_by.return_value.all.return_value = records
...
class TestTrainingRejection:
    ...
    def test_insufficient_data_raises(self, service, mock_db):
        ...
        for i in range(50):
            record = Mock()
            record.source_ip = f"10.0.0.{i}"
            record.volume = 10
            record.total_count = 100
            record.failed_count = 5
            record.unique_domains = 1
            record.first_seen = datetime(2024, 1, 1)
            record.last_seen = datetime(2024, 1, 2, i % 24)
            records.append(record)
    ...
    def test_exactly_100_samples_passes(self, service, mock_db):
        ...
        for i in range(100):
            record = Mock()
            record.source_ip = f"10.0.{i // 256}.{i % 256}"
            record.volume = np.random.randint(1, 50)
            record.total_count = 100
            record.failed_count = np.random.randint(0, 50)
            record.unique_domains = np.random.randint(1, 5)
            record.first_seen = datetime(2024, 1, 1)
            record.last_seen = datetime(2024, 1, 15, i % 24)
            records.append(record)
